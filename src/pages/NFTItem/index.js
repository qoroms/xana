...
import iconArtion from 'assets/svgs/Logo.svg';
...
const CHAIN = ENV === 'MAINNET' ? ChainId.MAINNET : ChainId.KOVAN;
...
const NFTItem = () => {
  ...
  const itemUpdatedHandler = async (owner, nft, id, paymentToken, newPrice) => {
    if (eventMatches(nft, id)) {
      await itemUpdated(owner, nft, id, paymentToken, newPrice, authToken);
      const token = getTokenByAddress(paymentToken);
  ...
  const itemCanceledHandler = async (owner, nft, id) => {
    if (eventMatches(nft, id)) {
      await itemCanceled(owner, nft, id, authToken);
  ...
  const offerCanceledHandler = async (creator, nft, id) => {
    if (eventMatches(nft, id)) {
      await offerCanceled(creator, nft, id, authToken);
