...
const wETHAddress = toLowerCase(process.env.WETH_ADDRESS);
...
const getDecimals = async (address) => {
  ...
  if (
    ...
    address = toLowerCase(process.env.WETH_ADDRESS);
...
const getSymbol = async (address) => {
  address = toLowerCase(address);
  if (address == "eth" || address == "Ethereum" || address == validatorAddress)
    return "ETH";
  if (address == "weth") address = toLowerCase(process.env.WETH_ADDRESS);
