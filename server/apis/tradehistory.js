...
router.post("/addBundleHistory", service_auth, async (req, res) => {
  try {
    ...
    newHistory.price = parseToETH(req.body.price);
    newHistory.paymentToken = parseToETH(req.body.paymentToken);
    let unitTkPrice = getPrice(req.body.paymentToken);
    newHistory.priceInUSD = parseToETH(req.body.price) * unitTkPrice;
